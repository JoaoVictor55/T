...
do universo de aprendizagem de máquina comumente adotadas eem metodologias da área.
...
aberto de ECG da história; estando disponível desde da década de 80, permitindo a comparação com demais trabalhos \cite{moody2001}. Além 
de ser recomendado pela AAMI.
...
Cada uma das anotações foram feitas por três cardiologistas independentes. Os sinais foram coletados com duas derivações; uma superior e outra inferior. A superior
...
Em sentido anti-horário, a partir do canto superior esquerdo tem-se: 

\begin{enumerate}
\item {TP} (\textit{true positive} - verdadeiro positivo), quantos casos positivos foram corretamente classificados;
\item {FN} (\textit{false negative} - falso negativo), quantos casos positivos foram incorretamente classificados;
\item {FP} (\textit{false positive} - falso positivo), quantos casos negativos foram incorretamente classificados;
\item {TN} (\textit{true negative} - verdadeiro negativo), quantos casos negativos foram incorretamente classificados.
\end{enumerate}

Com base nesses tipos, são definidas as seguintes métricas de desempenho
...
\noindent\textbf{Precisão:} Indica a proporção de batimentos classificados como arrítmicos que realmente pertencem à classe positiva:
\begin{equation}
...
Abaixo, é ilustrada uma curva precisão-\textit{recall} que  mostra o desempenho do modelo em todos os limiares de decisão possíveis e o 
efeito desse \textit{trade-off}:
...
não existe. O modelo sempre acerta, não importa o \textit{recall}. 

A AUCPR (\textit{area under curve precision recall} - ára sob a curva
precisão sensibilidade) é uma métrica que resume essas curvas. É possível comparar essa métrica com um \textit{baseline} que corresponde 
...
Uma alternativa a curva PR é a ROC (\textit{receive operator curve}) que exibe a sensibilidade em relação a taxa de falsos positivos. O seu 
comportamento é semelhante ao da curva PR, porém ela tende a ser otimista em conjunto de dados desbalanceados \cite{geron2022hands}.
Assim, neste trabalho foi optado por usar apenas a curva PR, mas, será discutido a AUC (\textit{area under the curve}) dos modelos. Essa métrica 
é equivalente a AUCPR, com a diferença que, para o \textit{baseline}, a AUC é sempre 0,5. Neste cenário, o \textit{recall} é igual à 
...
Um modelo perfeito, terá um AUC de 1,0.
...
a média dessa probabilidade e a frequência da classe positiva. Caso essas sejam iguais, então o modelo está perfeitamente calibrado.
